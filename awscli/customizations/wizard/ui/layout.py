...
class WizardDetailsDialogue:
    ...
    def _create_dialog(self):
        ...
        #             textfield,
        #             VSplit([save_button, cancel_button], padding=1)
        #         ],
        ...
        #     )
        # )
        # return Box(shadow, style='class:wizard.dialog')
        dialog = Dialog(
            title='Save to file',
            body=HSplit([
                Label(text='Filename', dont_extend_height=True),
    ...
    def _create_save_button(self, textfield):
        def save_handler(*args, **kwargs):
            app = get_app()
            buffer = app.layout.get_buffer_by_name('details_buffer')
