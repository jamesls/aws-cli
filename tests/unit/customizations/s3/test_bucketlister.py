...
class _BucketListTestBase:
    ...
    def test_list_objects_uses_local_tz_aware_datetimes_by_default(self):
        ...
        self.assertEqual(
            last_modified,
            parse(timestamp).astimezone(tzlocal()),
        )
        self.assertIsNotNone(last_modified.tzinfo)


class TestBucketList(_BucketListTestBase, unittest.TestCase):
    LISTER_CLS = BucketLister


class TestThreadedBucketList(_BucketListTestBase, unittest.TestCase):
